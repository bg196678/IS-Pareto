...
from reactor_model_system_2 import simulate_reactor


experimental_data = pd.read_excel("data/system_2/experimental.xlsx")
...
for index, row in experimental_data.iterrows():
    ...
    tres_min = row["t_res/min"]
    ratio = row["7:6"]
    concentration = row["Conc R1/M"] * 1000
...
plt.savefig("plots/system_2/reactor_model_analysis.png")
plt.tight_layout()
plt.show()
...
plt.tight_layout()
plt.savefig("plots/system_2/sty_correlation.png")
plt.show()
...
plt.xticks(rotation=0)
plt.tight_layout()
plt.savefig("plots/system_2/e_factor_correlation.png")
plt.show()
